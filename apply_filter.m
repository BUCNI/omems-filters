...
[file,location,indx] = uigetfile( ...
    ...
    'Audio/Sound files to filter (*.wav, *.flac, *.mp3, *.m4a, *.mp4, *.ogg, *.oga, *.opus)'}, ...
    'Select the audio file(s) you want to filter', ...
    'MultiSelect', 'on');

uiwait(msgbox("Select the correct filter file.", "modal"));
...
uiwait(msgbox("Select output directory. WARNING - can overwrite if output dir same as input dir", "modal"));
